...
- 入口：`main.py`，以 `uvicorn main:app` 啟動，預設 port `8003`（見 `~/claude_workspace/PORTS.md`）。
...
- AI 引擎選擇寫入 server config 的 `ai_engine`，所有前景功能與背景排程共用；`localStorage` 僅作顯示快取。無 API Key。
