...
# 模型可原生讀取（多模態）的檔案類型：PDF 與圖片。get_text() 只能抓文字層，遇到整頁
# 設計排版（封面、經營團隊、組織圖等常見版面）會抓到空字串；呼叫端可用此常數判斷是否
# 該把原始檔案路徑也交給模型讀圖，補足純文字抽取漏掉的內容。
NATIVE_EXTS = {".pdf", ".jpg", ".jpeg", ".png", ".gif", ".webp"}
