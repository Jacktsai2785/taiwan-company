...
步驟 4：根據前面搜尋所掌握的業務性質，搜尋台灣競爭者——**請涵蓋以下四種競業類型，四類的把關標準完全一致，無一例外**：每一類都**只填搜尋結果中能佐證真實存在的公司**（官網、媒體報導、公司登記均可）；查無實據時，該類別可以只填 0-1 家甚至留空，**嚴格禁止為了湊數而捏造、推測公司名稱，或把兩家不同公司的名稱／業務混合杜撰出一家不存在的公司**：
...
## 業務概況
（根據官網、104公司頁、媒體報導或其他可信來源撰寫，**格式固定如下**：
先用一句話（不超過 60 字）說明公司在做什麼、為誰解決什麼問題；
接著換行，逐項以粗體小標開頭、每項獨立一段（段與段之間空一行），依序填寫：
**主要產品**：…／**核心技術**：…／**主要客戶群**：…
某項查無資料則該項標「——」；若全無資料，各項標「——」並在段末一行說明資訊侷限。
...
標籤後的內容需具體指向該公司業務或產業，禁止只寫「市場競爭激烈」、「法規風險」等空泛語句）
...
def _build_materials_prompt(company: dict, materials_text: str = "", interview_text: str = "") -> str:
    ...
    _no_nest = (
        "【標記規則｜務必嚴格遵守】補充資訊只能用以下**兩種**寫法之一標記，每則只標一次：\n"
        "（甲）整段／整句開頭式：把標記放在該段或該句的**最前面**，標記之後整段都算補充。"
        "例：「（簡報補充）公司2024年營收約1億元，主要來自…」。\n"
        "（乙）行內括號式：把補充內容**整個包進同一組括號**、標記後緊接冒號。"
        "例：「主力產品為AI膜片（簡報補充：採強化奈米陶瓷、超薄高透明）」——只有括號內算補充。\n"
        "**嚴禁**把「（簡報補充）」當成句尾標籤掛在內容後面，例如「…③技術許可收取版稅（簡報補充）。」"
        "或「…廣告商（簡報補充）。」都是**錯誤**寫法（會被判讀成空白補充，內容遺失）。"
        "若整句／整段都是補充，請改用（甲）把標記移到句首。\n"
        "**嚴禁巢狀或重複**：（甲）開頭的段落內不可再出現任何「（簡報補充：…）」；同一句不要疊兩個標記。"
    )
    if has_interview:
    ...
    async with _CLAUDE_LOCK:
        for attempt in range(2):
            try:
                ...
                # 600s：DD memo 含 web 搜尋，慢日常態性貼近 7 分鐘；420s 撞頂會把
                # 已燒的 token 全數作廢（超時不重試），寬限比砍掉重來便宜
                raw = await asyncio.to_thread(
                    claude_client.ask, prompt, 600, _WEB_TOOLS, engine, 12, model
                )
                summary, blurb = _split_blurb(raw)
