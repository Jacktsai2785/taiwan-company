...
# 設計重點：
#   - 內容沒變就跳過：用完整備份集合的 manifest 比對，閒置日不堆重複檔
#   - 落地前先驗證 JSON 可解析：避免把壞資料存成「備份」
...
[ -f "$DATA_DIR/companies.json" ] || { echo "找不到 $DATA_DIR/companies.json，跳過（尚無資料）"; exit 0; }
...
for f in "${FILES[@]}"; do
  ...
  if ! "$PY" -c "import json,sys; json.load(open(sys.argv[1], encoding='utf-8'))" "$DATA_DIR/$f" 2>/dev/null; then
    echo "⚠ $f 無法解析為 JSON（可能正在寫入），本次跳過備份" >&2
    exit 1
  fi
done
...
# 2) 全部使用者資料都沒變才跳過
MANIFEST="$({
  for f in "${FILES[@]}"; do [ -f "$DATA_DIR/$f" ] && sha256sum "$DATA_DIR/$f"; done
  for d in "${DIRS[@]}"; do
    [ -d "$DATA_DIR/$d" ] || continue
    while IFS= read -r -d '' file; do sha256sum "$file"; done < <(find "$DATA_DIR/$d" -type f -print0 | sort -z)
  done
} | sha256sum | cut -d' ' -f1)"
HASH_FILE="$BACKUP_DIR/.last-data-manifest.sha256"
if [ -f "$HASH_FILE" ] && [ "$(cat "$HASH_FILE")" = "$MANIFEST" ]; then
  echo "使用者資料自上次備份未變更，跳過"
  exit 0
fi
...
for d in "${DIRS[@]}"; do
  [ -d "$DATA_DIR/$d" ] && EXISTING+=("$d")
done
tar -czf "$ARCHIVE" -C "$DATA_DIR" "${EXISTING[@]}"
echo "$MANIFEST" > "$HASH_FILE"
echo "✅ 已備份 ${#EXISTING[@]} 項使用者資料 → $ARCHIVE ($(du -h "$ARCHIVE" | cut -f1))"
